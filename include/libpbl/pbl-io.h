 ...
 */
typedef struct PblIOStream {
  PBL_TYPE_DEF_HELPER(PblStream_T_Base);
} PblIOStream_T;
...
__attribute__((unused)) extern PblIOStream_T *PBL_STDIN;
...
/**
 * @brief Displays the passed text and takes input.
...
/**
 * @brief Displays the passed text and takes a single key input.
 * @param display The display message.
 * @param end The end-character that should be displayed. If per default '\0'. This is the same logic as in Print!
...
// Creating the overhead and struct type for the Pbl-Function 'PblPrint'
PBL_FUNC_OVERHEAD(PblPrint, PblNone_T, PblString_T *out, PblIOStream_T *stream, PblChar_T *end)
